...
export async function getSalonApplications(status = "all") {
  if (import.meta.env.VITE_MOCK === "1") {
    const applications = status === "all"
      ? [...MOCK_SALON_APPLICATIONS]
      : MOCK_SALON_APPLICATIONS.filter((app) => app.status === status);
    return applications;
  }
...
export async function approveSalonApplication(applicationId, adminNotes = "") {
  if (import.meta.env.VITE_MOCK === "1") {
    ...
    return { success: true, message: "Salon application approved successfully", salonId: `salon-${applicationId}` };
  }
...
export async function rejectSalonApplication(applicationId, rejectionReason) {
  if (import.meta.env.VITE_MOCK === "1") {
    await new Promise((resolve) => setTimeout(resolve, 500));
    return { success: true, message: "Salon application rejected successfully" };
  }
...
const MOCK_PLATFORM_METRICS = {
  ...
  peakTraffic: "13:00",
  activeUsers: 3247,
  databaseQueries: 98234,
  errorRate: 0.3,
  queueDepth: 12,
  services: [
    { name: "API", status: "healthy", latency: "142ms" },
    { name: "Database", status: "healthy", latency: "21ms" },
    { name: "Cache", status: "healthy", latency: "7ms" },
    { name: "Notifications", status: "degraded", latency: "412ms" },
  ],
};
